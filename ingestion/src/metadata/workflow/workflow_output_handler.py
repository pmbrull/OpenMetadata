...
def print_workflow_summary(workflow: BaseWorkflow) -> None:
    ...
    for step in [workflow.source] + list(workflow.steps):
        ...
        failures.append(Failure(name=get_generic_step_name(step), failures=step.get_status().failures))

        log_ansi_encoded_string(bold=True, message=f"Workflow {get_generic_step_name(step)} Summary:")
